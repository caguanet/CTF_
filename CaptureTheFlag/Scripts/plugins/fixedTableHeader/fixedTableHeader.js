...
(function ($) {
    $.fn.fixedTableHeader = function () {
        return this.each(function (key, item) {
            ...
            if ($.isEmptyObject(originalWidth) || originalWidth.length === 0) {
            ...
            function init() {
                ...
                $fixedHeader.css('min-height', $self.find('thead').outerHeight());
            ...
            init();
            $(".fixedtableheader").resize(onResize);
            $(".fixedtableheader").scroll(onScroll);
        });
    };
}(jQuery));